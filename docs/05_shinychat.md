...
## Заключение
Мы рассмотрели возможности пакета `shinychat`, которые позволяют превратить простой чат в полноценное бизнес-приложение с красивым UI и подсказками.

Но красивый интерфейс — это только половина успеха. В реальных бизнес-задачах пользователи часто хотят не просто получать текстовые ответы, а взаимодействовать с табличными данными: фильтровать их, строить графики и задавать вопросы вроде «Какая прибыль была в прошлом месяце?».

Чтобы научить наш чат понимать структуру данных и переводить вопросы пользователя в SQL-запросы, нам понадобится специализированный инструмент. В следующем уроке мы разберем пакет **`querychat`**, который создан именно для манипуляции данными на естественном языке.
